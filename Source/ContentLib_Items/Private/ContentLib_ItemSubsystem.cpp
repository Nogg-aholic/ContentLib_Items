...
TSharedRef<FJsonObject> FContentLib_ItemVisualKit::GetAsJsonObject(const TSubclassOf<UFGItemDescriptor> Item)
{
	...
	ColorGas->Values.Add("r",FluidColor_R);
	ColorGas->Values.Add("g",FluidColor_G);
	ColorGas->Values.Add("b",FluidColor_B);
	ColorGas->Values.Add("a",FluidColor_A);
	...
	if(Result->HasField("FluidColor") && Result->TryGetField("FluidColor")->Type == EJson::Object)
	{
		...
		if(Obj->HasField("r") && Obj->HasField("g") && Obj->HasField("b") && Obj->HasField("a"))
		{
			...
			if(Result->TryGetField("b")->Type == EJson::Number)
				Item.FluidColor.R = Result->TryGetField("b")->AsNumber();
			else
			{
...
FContentLib_ResourceItem::FContentLib_ResourceItem(): CollectSpeedMultiplier(-1), Valid(false)
{
	PingColor = FColor();
}
...
FContentLib_Item::FContentLib_Item(): Form(EResourceForm::RF_INVALID), EnergyValue(-1), RadioactiveDecay(-1), CanBeDiscarded(-1), RememberPickUp(-1),
...
FString FContentLib_Item::GenerateFromClass(const TSubclassOf<UFGItemDescriptor> Item)
{
	...
	Obj->Values.Add("ItemCategory",ItemCategory);
...
FContentLib_Item FContentLib_Item::GenerateFromString(const FString String)
{
	...
	if(Result->HasField("ItemCategory") && Result->TryGetField("ItemCategory")->Type == EJson::String)
	{
		Item.ItemCategory = Result->TryGetField("ItemCategory")->AsString();
	}
...
void FContentLib_Item::ApplyFromStruct(TSubclassOf<UFGItemDescriptor> Item,const UContentLib_ItemSubsystem * Subsystem) const
{
	...
	if(ItemCategory != "")
	{
		...
		{
			UClass * Loaded = LoadObject<UClass>(nullptr, *ItemCategory);
		...
		else
		{
			...
			{
				FString S = e->GetName();
				FString A = ItemCategory;
				FString DescPre = FString("Cat_").Append(ItemCategory);
				if (!DescPre.EndsWith("_C"))
				{
					DescPre.Append("_C");
				}
				if (S.Equals(ItemCategory, ESearchCase::IgnoreCase) || S.Equals(A.Append("_C"), ESearchCase::IgnoreCase) || S.Equals(DescPre, ESearchCase::IgnoreCase))
				{
					CDO->mItemCategory = e;
					break;
				}
	...
	if(CanBeDiscarded != -1)
	{
		CDO->mCanBeDiscarded = bool(CanBeDiscarded);
	}
	
	if(RememberPickUp != -1)
	{
		CDO->mRememberPickUp =  bool(RememberPickUp);
	}
	...
	if(Item->IsChildOf(UFGResourceDescriptor::StaticClass()))
	{
		if(ResourceItem.PingColor != FColor())
		...
		if(FuelWasteItem.SpentFuelClass != "")
		{
			...
			else
			{
				for(auto e : Subsystem->Items)
				{
					...
					if (S.Equals(FuelWasteItem.SpentFuelClass, ESearchCase::IgnoreCase) || S.Equals(A.Append("_C"), ESearchCase::IgnoreCase) || S.Equals(DescPre, ESearchCase::IgnoreCase))
					{
...
void UContentLib_ItemSubsystem::ApplyFromStruct(const TSubclassOf<UFGItemDescriptor> Item, const FContentLib_Item Struct)
{
...
TSubclassOf<UFGItemDescriptor> UContentLib_ItemSubsystem::CreateContentLibItem(FString Name,TSubclassOf<UFGItemDescriptor> Class)
{
